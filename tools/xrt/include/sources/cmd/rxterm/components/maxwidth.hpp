...
namespace rxterm {
    ...
    struct MaxWidth {
        ...
        MaxWidth(std::function<unsigned(unsigned)> maxWidth, Component c) : maxWidth{std::move(maxWidth)},
                c{std::move(c)} {}

        MaxWidth(unsigned const &maxWidth, Component c) : maxWidth{px(maxWidth)}, c{std::move(c)} {}
