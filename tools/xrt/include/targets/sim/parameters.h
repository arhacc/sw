...
//^ must be power of 2, greater or equal to 2 ; for dte column_counter+padding_counter
//generation; should be fixed (and possibly others) ^ currently, for axi stream
//integration in io, should be fixed at 2. untested other values at that level
#define NR_CELLS_PER_MULTICELL \
    (NR_CELLS_PER_PARTIAL_MULTICELL * NR_PARTIAL_MULTICELLS_PER_MULTICELL)
...
//!!!!!!!!!!!!!!!!!!!! comment/uncomment as needed !!!!!!!!!!!!!!!!! changing value to 0
//!has no effect
...
// ^^ requires RESOURCE_ARRAY_CELL_HAS_ROTATE and RESOURCE_ARRAY_CELL_HAS_SHIFT in order
// to function float operations require data sizes of 32b; div in ctrl not implemented
...
// ^ active only with IO_DATA_FIFO_USE_PRIMITIVE defined; can be: "auto", "block",
// "distributed", "ultra"
...
// imposes this number of clock cycles between 2 succesive shift instructions for data
// validity
...
// if commented, select which scan mode using RESOURCE_NETWORK_SINGLE_SCAN_MODE further
// below
#define NETWORK_SCAN_MODE_CONTINOUS_MEM_OUT 0
#define NETWORK_SCAN_MODE_CONTINOUS_GLOBAL_SHIFT_REG \
    (NETWORK_SCAN_MODE_CONTINOUS_MEM_OUT + 1)
#define NETWORK_SCAN_MODE_ONE_SHOT_MEM_OUT \
    (NETWORK_SCAN_MODE_CONTINOUS_GLOBAL_SHIFT_REG + 1)
