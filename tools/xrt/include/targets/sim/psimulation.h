...
// ^ wait amount after set interrupt; disabled if "SIMULATION_NR_DATA_OUT_READ is not
// known
#define SIMULATION_DRAIN_TIME_DATA_OUT_CLOCK_CYCLES (32 * `ARRAY_NR_CELLS * 2)
// ^ wait amount after set interrupt. in order to read data out when
// "SIMULATION_NR_DATA_OUT_READ" is not known
...
// ^ comment/uncomment; use when in depth debug is needed, do not use with post synth
// simulation
...
// if printing speicfic locations form memory is needed, modify custom printing array (top
// of file) in simulator
