...
namespace rxterm {
    ...
    template<class T = Component> struct FlowLayout {
        std::vector<T> children;
        Pixel bg;

        FlowLayout(std::vector<T> const &children, Pixel bg = Pixel{}) : children(children), bg{bg} {}

        template<class...Xs> FlowLayout(Xs const &...xs)
                : children{xs...}, bg{Pixel{}} {}

        template<class...Xs> FlowLayout(Pixel const &bg, Xs const &...xs)
                : children{xs...}, bg{bg} {}
