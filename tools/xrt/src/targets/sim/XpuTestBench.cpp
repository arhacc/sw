...
xsi(design_libname, simkernel_libname)
{
    ...
    clock_half_period = (unsigned int)(clock_period_ns*pow(10,-9)/xsi.get_time_precision()/2);
...
void XpuTestBench::list_ports()
{
    ...
    /*Uncomment here if you want your ports listed*/
...
void XpuTestBench::write(const std::string &port_name, unsigned int val)
{
    ...
    unsigned int nwords = (port_map[port_name].port_bits+31)/32;
...
unsigned int XpuTestBench::read(const std::string &port_name)
{
    ...
    if(nwords > 1)
    {
        throw std::invalid_argument("uint = read(string name) applies only to signals of 64b or less");
    }
    std::vector<s_xsi_vlog_logicval> logic_val(nwords);
...
{
    xsi.put_value(port_map["resetn"].port_id, &zero_val);
    wait_clock_cycles(nr_active_clock_cycles);
    xsi.put_value(port_map["resetn"].port_id, &one_val);
}
...
void XpuTestBench::AXI_init()
{
    ...
    for(int i=0; i<xsi.get_num_ports(); i++)
    {
        if (port_map[static_cast<std::string>(xsi.get_port_name(i))].is_input && std::regex_match(xsi.get_port_name(i), regex_axi) )
        {
...
void XpuTestBench::AXI_Lite_write(unsigned int waddr, unsigned int wdata)
{
    //posedge clock
    write_addr(waddr);
    ...
    write_data(wdata);
    //write("s00_axi_wstrb", IO_INTF_PROG_AXILITE_DATA_SIZE/8 ); /*what is this ? */
    ...
    std::cout << "written at: " << std::hex << "(0x" << waddr << ") data: "
              << std::hex << "(0x" << wdata << ")" << std::endl;
}
...
int main()
{
    std::string design_libname = "/home/alex13dumi/gitroot/xpu/sw/tools/xrt/build/xsim.dir/xsim/xsimk.so";
	XpuTestBench xpu(design_libname, "librdi_simulator_kernel.so", 1, "clock", "resetn");
	xpu.tb_init();
    xpu.do_reset(10);
    xpu.wait_clock_cycles(30);
    xpu.AXI_Lite_write(200,300);
    xpu.AXI_Lite_write(400,500);
}
*/