...
XpuTestBench::XpuTestBench(
    ...
    clock_half_period = (unsigned int) (clock_period_ns * 10 * pow(10, -9)
                                        / xsi.get_time_precision() / 2);
    ...
    list_ports();
    std::cout << "Using " << clock << " as clock with half-period of "
              << clock_half_period << " simulation steps" << std::endl;
    std::cout << "Using " << reset << " as "
              << "reset" << std::endl;
...
bool comparator(
    std::pair<std::string, XpuTestBench::port_parameters>& x,
...
sort(const std::map<std::string, XpuTestBench::port_parameters>& ports) {
    std::vector<std::pair<std::string, XpuTestBench::port_parameters>> copy_ports(
...
bool XpuTestBench::axi_bvalid_ready() {
    ...
    std::cout << "time:" << std::dec << xsi.get_time() << " bvalid value: " << bvalid
              << std::endl;
    return (bvalid & 0x1) != 0;
}
...
void XpuTestBench::tb_init() {
    std::string simulation_accelerator_setup(
        std::to_string(ARRAY_NR_CELLS) + '_'
        + std::to_string(NR_PARTIAL_MULTICELLS_PER_MULTICELL) + '_'
    ...
    std::string axi_lite_program_file_combined_path(
        SIMULATION_FILE_PATH_BACKSTEPS
        + "io_simulation_data/program_file/program_file_combined_"
        + simulation_accelerator_setup + "_" + SIMULATION_FUNCTION_PERFORMED + "_"
        + SIMULATION_TEST_NAME + ".txt");

    std::string axi_lite_function_arguments_file_path(
        SIMULATION_FILE_PATH_BACKSTEPS
        + "io_simulation_data/function_arguments/function_arguments_"
        + simulation_accelerator_setup + "_" + SIMULATION_FUNCTION_PERFORMED + "_"
        + SIMULATION_TEST_NAME + ".txt");

    std::string axi_stream_in_file_path(
        SIMULATION_FILE_PATH_BACKSTEPS + "io_simulation_data/input/axi_stream_input_"
        + simulation_accelerator_setup + "_" + SIMULATION_FUNCTION_PERFORMED + "_"
        + SIMULATION_TEST_NAME + ".txt");
    ...
    std::string golden_model_file_path(
        SIMULATION_FILE_PATH_BACKSTEPS
...
#endif
    axi_lite_program_file_combined.open(
        axi_lite_program_file_combined_path, std::ios::out);
    axi_lite_program_file.open(axi_lite_program_file_path, std::ios::in);
    axi_lite_function_arguments_file.open(
        axi_lite_function_arguments_file_path, std::ios::in);
...
void XpuTestBench::AXI_init() {
    ...
    for (int i = 0; i < xsi.get_num_ports(); i++) {
        if (port_map[static_cast<std::string>(xsi.get_port_name(i))].is_input
            ...
            std::cout << i << " " << xsi.get_port_name(i)
                      << " value: " << read(xsi.get_port_name(i)) << std::endl;
        }
    }
...
void XpuTestBench::AXI_Lite_write(uint32_t waddr, uint32_t wdata) {
    ...
    ++axi_write_count;
    std::cout << "written at: " << std::hex << "(0x" << waddr << ") data: " << std::hex
              << "(0x" << wdata << ")" << std::endl;
}
