...
void WriteProgramThread::run() {
    ...
    for( ; ; ) /*for iterating in program data vector*/
    {
        AxiStreamWrite(0, prog_data_controller); /*if the vector contains prog data from controller and array one after
                                                              *another we can leave only v[i];*/
        AxiStreamWrite(0, prog_data_array);

        //Create a bitset class for easier comparations
        prog_data_operand = extractBits(prog_data_controller,
                                        INSTR_OPERAND_LOC_UPPER-INSTR_OPERAND_LOC_LOWER+1,
                                        INSTR_OPERAND_LOC_LOWER);


        prog_data_opcode = extractBits(prog_data_controller,
