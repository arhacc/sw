...
makeInstructionByte(const Arch& _arch, uint8_t _opcode, uint8_t _operand) {
    assert(_arch.get(ArchConstant::INSTR_OPCODE_NR_BITS) == 5);
    assert(_arch.get(ArchConstant::INSTR_OPERAND_NR_BITS) == 3);
...
makeInstruction(const Arch& _arch, uint8_t _instructionByte, uint32_t _argument) {
    assert(
        _arch.get(ArchConstant::INSTR_OPCODE_NR_BITS)
            + _arch.get(ArchConstant::INSTR_OPERAND_NR_BITS)
        == 8);
    assert(_arch.get(ArchConstant::INSTR_VALUE_NR_BITS) == 24);

    return static_cast<uint32_t>(_instructionByte) << 24 | (_argument & 0xFF'FF'FF);
}

//-------------------------------------------------------------------------------------
static inline uint32_t makeJumpInstruction(
    const Arch& _arch,
    uint8_t _instructionByte,
    uint8_t _jmpFunction,
    uint8_t _valReg,
    uint16_t _value) {
    assert(_arch.get(ArchConstant::INSTR_JMP_FUNCTION_NR_BITS) == 6);
    assert(_arch.get(ArchConstant::INSTR_JMP_FUNCTION_BR_w_VAL_NR_BITS) == 3);
    assert(_arch.get(ArchConstant::INSTR_JMP_VALUE_NR_BITS) == 15);
