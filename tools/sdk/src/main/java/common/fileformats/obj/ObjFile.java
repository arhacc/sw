...
//-------------------------------------------------------------------------------------
public class ObjFile extends XpuFile {
    protected ArrayList<ObjSegment> featureSegments = new ArrayList<>();
    protected ArrayList<ObjSegment> codeSegments = new ArrayList<>();
    protected ArrayList<ObjSegment> dataSegments = new ArrayList<>();
...
//-------------------------------------------------------------------------------------
    public ObjFile(Logger _log, String _path, List<Program> _programs, List<Data> _datas, List<Long> _features) {
        super(_log, _path, EXTENSION);
        ObjSegment _featureSegment = new ObjSegment(log, -1);
        _featureSegment.setData(_features);

        int _address = 0;
        ObjSegment _codeSegment = new ObjSegment(log, _address);
...
//                _hex.add(_address, _hexLine);
        ...
        ObjSegment _dataSegment = new ObjSegment(log);
...
//-------------------------------------------------------------------------------------
    public ArrayList<ObjSegment> getFeatureSegments() {
        return featureSegments;
    }

//-------------------------------------------------------------------------------------
    public ArrayList<ObjSegment> getCodeSegments() {
        return codeSegments;
    }

//-------------------------------------------------------------------------------------
    public ArrayList<ObjSegment> getDataSegments() {
        return dataSegments;
    }
...
//-------------------------------------------------------------------------------------
    public ObjSegment getFeatureSegment(int _index) {
    return featureSegments.get(_index);
}
    public ObjSegment getFeatureSegment() {
        return featureSegments.get(0);
    }
    public ObjSegment getCodeSegment(int _index) {
    return codeSegments.get(_index);
}
    public ObjSegment getCodeSegment() {
    return codeSegments.get(0);
}
    public ObjSegment getDataSegment(int _index) { return dataSegments.get(_index);}
    public ObjSegment getDataSegment() { return dataSegments.get(0);}

    public void addFeatureSegment(ObjSegment _featureSegment) { featureSegments.add(_featureSegment);}
    public void addCodeSegment(ObjSegment _codeSegment) { codeSegments.add(_codeSegment);}
    public void addDataSegment(ObjSegment _dataSegment) {
...
//          log.info("Loading [" + _index + ":" + _filePath + "]...OK[" + _data.size() + " entries]");
     ...
     */
    private ArrayList<ObjSegment> readSegments(Logger _log, Input _input) {
        ...
        while (num-- > 0) {
            ...
            ret.add(new ObjSegment(_log, address, data));
        }
        return ret;
    }
     ...
     */
    private void saveSegments(ArrayList<ObjSegment> _arr, Output _output) {
        crcValue ^= _arr.size();
